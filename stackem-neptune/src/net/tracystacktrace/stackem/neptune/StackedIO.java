...
public class StackedIO {
    ...
    public StackedIO(@NotNull List<@NotNull File> texturepackArchives) {
    ...
    public @NotNull InputStream getInputStream(@NotNull String resourcePath) throws IOException {
        this.initialize();
        for (int i = 0; i < this.archives.length; i++) {
    ...
    public @Nullable URL find(@NotNull String resourcePath) {
        try {
            this.initialize();
        } catch (Exception e) {
            return null;
    ...
    public boolean fileExists(@NotNull String resourcePath) {
        try {
            this.initialize();
            for (int i = 0; i < this.archives.length; i++) {
                final ZipEntry entry = this.archives[i].getEntry(resourcePath);
    ...
    public boolean directoryExists(@NotNull String resourcePath) {
        try {
            this.initialize();
            for (int i = 0; i < this.archives.length; i++) {
                final ZipEntry entry = this.archives[i].getEntry(resourcePath);
